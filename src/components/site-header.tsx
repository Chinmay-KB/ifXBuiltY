...
type Props = {
  user: User | null;
  active?: "specimen" | "generate" | "images" | "feed";
  ...
  return (
    <header className="flex h-[72px] w-full shrink-0 items-center justify-between gap-4 border-b border-line bg-canvas px-4 sm:px-12">
      ...
      <nav className="flex items-center gap-3 sm:gap-6">
        <NavTextLink href="/" active={active === "specimen"} className="hidden sm:inline">
          Specimen view
        </NavTextLink>
      ...
      <div className="flex shrink-0 items-center gap-2 sm:gap-3">
        {primaryAction ? (
          <Link
            ...
            className="inline-flex items-center justify-center rounded-lg bg-[#E8E306] px-3.5 py-2.5 text-sm font-black leading-5 text-ink transition-colors hover:bg-[#E8E306]/90"
          >
            {primaryAction.label}
          </Link>
        ) : null}
